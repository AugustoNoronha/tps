...
class Lista {
    ...
    //     return resp;
    // }
    public void callquicksort() throws Exception{
       quickSort(array, 0, n-1);
    }
    public void quickSort(Games[] v, int esquerda ,int direita) throws Exception {
        int esq = esquerda;
        int dir = direita;
        ...
        while (esq <= dir) {
            while (v[esq].getDate().before(pivo.getDate())) {
                esq = esq + 1;
            }
            while (v[dir].getDate().after(pivo.getDate())) {
                dir = dir - 1;
            }
            if (esq <= dir) {
                troca = array[esq];
