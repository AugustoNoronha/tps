...
class Q01 {
	...
	public static void main(String[] args) throws Exception {
        ...
        Scanner scanner = new Scanner(new File("E:\\Facudade\\tps\\tp2\\games.csv"));
		...
		// Standard process of getting input
		String[] input = new String[52];

		int index = 0;

		do { input[index] = MyIO.readLine(); } while (!endOfTest(input[index++]));

		// Mapping the desired games
		for (int i = 0; i < index-1; i++) for (int j = 0; j < pos; j++) {

			if (Integer.parseInt(input[i]) == games[j].getAppId()) games[j].print();
		}
	}
